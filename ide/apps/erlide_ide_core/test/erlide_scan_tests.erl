...
tokens_test_() ->
    [
        ?_assertEqual(
            {ok, [#token{kind = atom, line = 0, offset = 0, length = 1, value = a, text = "a"}],
                {0, 2, 1}},
            test_scan("a")
        ),
        ?_assertEqual(
            {ok, [#token{kind = var, line = 0, offset = 0, length = 1, value = 'A', text = "A"}],
                {0, 2, 1}},
            test_scan("A")
        ),
        ?_assertEqual(
            {ok,
                [#token{kind = macro, line = 0, offset = 0, length = 2, value = '?a', text = "?a"}],
                {0, 3, 2}},
            test_scan("?a")
        ),
        ?_assertEqual(
        ...
        ?_assertEqual(
            {ok,
                [
                    #token{
                        kind = macro, line = 0, offset = 0, length = 3, value = '??a', text = "??a"
                    }
                ],
                {0, 4, 3}},
            test_scan("??a")
        ...
        ?_assertEqual(
            {ok, [#token{kind = white_space, line = 0, offset = 0, length = 1, text = <<" ">>}],
                {0, 2, 1}},
            test_scan(" ")
        ),
        ?_assertEqual(
        ...
        ?_assertEqual(
            {ok,
                [#token{kind = char, line = 0, offset = 0, length = 3, text = "$\\s", value = $\s}],
                {0, 4, 3}},
            test_scan("$\\s")
        ),
        ?_assertEqual(
        ...
        ?_assertEqual(
            {ok, [#token{kind = integer, line = 0, offset = 0, length = 1, text = "3", value = 3}],
                {0, 2, 1}},
            test_scan("3")
        ),
        ?_assertEqual(
        ...
        ?_assertEqual(
            {ok,
                [
                    #token{
                        kind = atom, line = 0, offset = 0, length = 5, text = "'try'", value = 'try'
                    }
                ],
                {0, 6, 5}},
            test_scan("'try'")
        ...
        ?_assertEqual(
            {ok, [#token{kind = comment, line = 0, offset = 0, length = 3, text = <<"% h">>}],
                {0, 4, 3}},
            test_scan("% h")
        ),
        ?_assertEqual(
            {ok,
                [#token{kind = float, line = 0, offset = 0, length = 3, text = "2.5", value = 2.5}],
                {0, 4, 3}},
            test_scan("2.5")
        ),
        ?_assertEqual(
        ...
        ?_assertEqual(
            {ok, [#token{kind = dot, line = 0, offset = 0, length = 1, text = "."}], {0, 2, 1}},
            test_scan(".")
        ),
        ?_assertEqual(
            {ok, [#token{kind = 'try', line = 0, offset = 0, length = 3, text = "try"}], {0, 4, 3}},
            test_scan("try")
        )
    ].
    ...
    [
        ...
        ?_assertEqual(
            {ok,
                [
                    #token{
                        kind = float, line = 0, offset = 0, length = 3, text = "2.5", value = 2.5
                    },
        ...
        ?_assertEqual(
            {ok,
                [
                    ...
                    #token{kind = white_space, line = 0, offset = 1, length = 1, text = <<" ">>},
                    #token{
                        kind = macro, line = 0, offset = 2, length = 2, text = "?a", value = '?a'
                    },
        ...
        ?_assertEqual(
            {ok, [#token{kind = atom, line = 3, offset = 5, length = 1, value = a, text = "a"}],
                {3, 5, 6}},
            test_scan("a", 3, 4, 5)
        )
    ].
...
unicode_test_() ->
    [
        ?_assertEqual(
            {ok, [{token, char, 0, 0, 9, $\x{0206}, "$\\x{0206}", u, u}], {0, 10, 9}},
            test_scan("$\\x{0206}")
        ),
        ?_assertEqual(
            {ok, [{token, string, 0, 0, 10, "\x{0206}", "\"\\x{0206}\"", u, u}], {0, 11, 10}},
            test_scan("\"\\x{0206}\"")
        ),
        ?_assertEqual({ok, [{token, char, 0, 0, 2, $Ȇ, "$Ȇ", u, u}], {0, 3, 2}}, test_scan("$Ȇ")),
        ?_assertEqual(
            {ok, [{token, string, 0, 0, 3, "Ȇ", "\"Ȇ\"", u, u}], {0, 4, 3}}, test_scan("\"Ȇ\"")
        ),
        ?_assertEqual({ok, [{token, char, 0, 0, 2, $生, "$生", u, u}], {0, 3, 2}}, test_scan("$生")),
        ?_assertEqual(
            {ok, [{token, string, 0, 0, 3, "生", "\"生\"", u, u}], {0, 4, 3}}, test_scan("\"生\"")
        ),
        ?_assertEqual(
            {ok, [{token, comment, 0, 0, 2, u, <<"%生"/utf8>>, u, u}], {0, 3, 2}}, test_scan("%生")
        )
    ].
    ...
    [
        ?_assertEqual(
            {ok,
                [
                    #token{
                        kind = string, line = 0, offset = 0, length = 3, value = "b", text = "\"b\""
                    },
                    #token{kind = white_space, line = 0, offset = 3, length = 1, text = <<"\n">>},
                    #token{
                        kind = string, line = 1, offset = 4, length = 3, value = "b", text = "\"b\""
                    },
