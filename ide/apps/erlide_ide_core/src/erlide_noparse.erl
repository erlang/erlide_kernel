...
get_tokens(ScannerName, ModuleFileName, InitialText, StateDir) ->
    case whereis(ScannerName) of
        undefined ->
            {_Cached, Module} = erlide_scanner:initial_scan_0(
                ScannerName, ModuleFileName, InitialText, StateDir, true
            ),
...
do_parse(ScannerName, RefsFileName, Tokens, StateDir, UpdateSearchServer) ->
    ...
    case StateDir of
        "" -> ok;
        _ -> file:write_file(RefsFileName, term_to_binary(References, [compressed]))
    end,
