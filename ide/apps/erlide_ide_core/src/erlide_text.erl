...
check_function_call(
    [
        #token{kind = atom, value = M},
        #token{kind = ':'},
        #token{kind = atom, value = F},
        #token{kind = '('}
        | Rest
    ],
    Index
    ...
    ?D({Index, Rest}),
    {ok, M, F, Rest};
check_function_call([#token{kind = atom, value = F}, #token{kind = '('} | Rest], Index) when
    Index > -1
->
    ?D({Index, F}),
    {ok, F, Rest};
...
split_lines("", _Pos, _LinePos, "", Acc) ->
    Acc;
split_lines([$$, C | Rest], Pos, LinePos, LineAcc, Acc) when
    % fix for 1193
    C =:= $\n; C =:= $\r
->
...
clean_tokens([#token{kind = '?'} = Q, #token{kind = Kind, value = Value} | Rest], W, I, Acc) when
    Kind == atom; Kind == var
->
...
initial_whitespace([C | S], N) when C =:= $\s; C =:= $\t ->
...
entab(S, Tablength, left) ->
    ...
    lists:append([string:chars($\t, N div Tablength), string:chars($\s, N rem Tablength), Rest]).
