...
-export([
    ...
    attributes_info/1, attributes_info/2,
    set_attribute/3
]).
...
-opaque return_cont() ::
    {erl_scan_continuation, string(), column(), tokens(), line(), #erl_scan{}, any(), cont_fun()}.
-type tokens_result() ::
...
token_info(Token) ->
    % undefined order
    Items = [category, column, length, line, symbol, text],
    token_info(Token, Items).

-spec token_info
    (Token, TokenItem) -> TokenInfoTuple | 'undefined' when
        Token :: token(),
...
attributes_info({Line, Column}, column = Item) when
    ?ALINE(Line),
    ?COLUMN(Column)
->
    {Item, Column};
...
attributes_info(Line, line = Item) when ?ALINE(Line) ->
    {Item, Line};
attributes_info({Line, Column}, line = Item) when
    ?ALINE(Line),
    ?COLUMN(Column)
->
    {Item, Line};
attributes_info(Attrs, line = Item) ->
    attr_info(Attrs, Item);
attributes_info({Line, Column} = Location, location = Item) when
    ?ALINE(Line),
    ?COLUMN(Column)
->
    {Item, Location};
