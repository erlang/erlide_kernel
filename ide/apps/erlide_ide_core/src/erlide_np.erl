...
classify_and_collect_forms([C | Rest], Acc, RefsAcc, Exports, Imports) ->
    ...
    classify_and_collect_forms(
        Rest, [R | Acc], Refs ++ RefsAcc, MoreExports ++ Exports, MoreImports ++ Imports
    ).
...
cac_form(
    other,
    [#token{value = Name, line = Line, offset = Offset, length = Length} | _] = T,
    _Exports,
    _Imports
) ->
...
get_type_attribute(Name, Offset, Line, Attribute, Args) ->
    ...
    {
        #attribute{
            pos = {{Line, LastLine, Offset}, PosLength},
            name = Name,
            args = AttrArgs,
            extra = Extra,
            arity = Arity
...
get_function(Tokens, Exports, Imports) ->
    ...
    [{#clause{pos = P, name = Name, args = Arguments, head = H, name_pos = NP}, _Refs} | _] =
        ClausesAndRefs,
    Arity = length(Arguments),
    Exported = lists:member({Name, Arity}, Exports),
    % only show subclauses when more than one
    Function =
        case ClausesAndRefs of
            [_] ->
                #function{
                    pos = P,
                    name = Name,
...
get_attribute(
    [
        ...
        #token{kind = atom, line = _Line, offset = _Offset, value = Name}
        | Args
    ] = Attribute
) when
    (Name =:= 'type') or (Name =:= 'spec') or (Name =:= 'opaque')
->
    %% -spec, -type or -opaque
    get_type_attribute(Name, Offset, Line, Attribute, Args);
get_attribute(
...
check_class([#token{kind = AtomOrMacro}, #token{kind = '('} | _]) when
    AtomOrMacro =:= atom; AtomOrMacro =:= macro
->
    function;
...
fun_arity_from_tokens([
    #token{kind = atom, value = Fun},
    #token{kind = '/'},
    #token{kind = integer, value = Arity}
    | Rest
]) ->
...
split_clauses([#token{kind = Kind} = T | Rest], _HaveWhen, Acc, ClAcc) when
    Kind =:= 'end'; Kind =:= dot; Kind =:= '->'; Kind =:= 'when'
->
...
get_clause([
    #token{kind = AtomOrMacro, value = Name, line = Line, offset = Offset, length = Length} | Rest
]) when
    AtomOrMacro =:= atom; AtomOrMacro =:= macro
->
...
fix_code_refs(
    [], #function{name = Name, arity = Arity, name_pos = {{_, Offset}, Length}}, _Imports, Acc
) ->
    Ref = #ref{
        data = #function_def{function = Name, arity = Arity},
        offset = Offset,
        length = Length,
        function = Name,
        arity = Arity,
...
fix_code_refs([], _Clause, _Function, _Imports, Acc) ->
    Acc;
fix_code_refs(
    [{Offset, Length, RefData} | Rest],
    #clause{head = Head} = Clause,
...
get_refs_in_code(
    [
        #token{kind = atom, value = M, offset = Offset},
        #token{kind = ':'},
    ...
    R =
        {Offset, Length2 + Offset2 - Offset, #external_call{
            module = M, function = F, arity = Arity
        }},
    get_refs_in_code(Rest, [R | Acc]);
get_refs_in_code(
    [
        #token{kind = atom, value = F, offset = Offset, length = Length},
        #token{kind = '('}
        | Rest
    ],
    Acc
    ...
    get_refs_in_code(Rest, [R | Acc]);
get_refs_in_code(
    [#token{kind = macro, value = M, offset = Offset, length = Length} | Rest],
    Acc
) ->
...
get_refs_in_code(
    [
        #token{kind = '#', offset = Offset},
        #token{kind = atom, value = Record, offset = Offset2, length = Length2}
        | Rest
    ],
    Acc
) ->
...
get_refs_in_code(
    [#token{kind = var, value = V, offset = Offset, length = Length} | Rest],
    Acc
) ->
...
get_refs(
    ...
    Ref = make_ref(
        Offset,
        Length2 + Offset2 - Offset,
        #type_ref{module = M, type = T},
        Name,
        Arity
    ),
...
get_refs(
    [
        #token{kind = atom, value = T, offset = Offset, length = Length},
        #token{kind = '('}
        | Rest
    ],
    Name,
    ...
    get_refs(Rest1, Name, Arity, [Ref | Acc]);
get_refs(
    [#token{kind = variable, value = V, offset = Offset, length = Length} | Rest],
    Name,
    Arity,
    Acc
) ->
...
get_record_field_defs([], _) ->
    [];
get_record_field_defs(
    [{FieldName, {{_, _, Offset}, Length}, _Extra} | Rest],
    RecordName
) ->
    [
        make_ref(
            Offset,
            Length,
            #record_field_def{record = RecordName, name = FieldName},
            FieldName,
            ?ARI_RECORD_FIELD_DEF
        )
