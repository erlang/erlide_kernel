...
find_data(
    [
        #ref{function = F, arity = A, clause = C, data = D, offset = O, length = L, sub_clause = S}
        | Rest
    ],
    Pattern,
    Mod,
    ...
    NewAcc =
        case check_pattern(Pattern, Mod, D, F, A, C) of
            true ->
...
check_function_ref(_, []) ->
    false;
check_function_ref(#external_call{module = Mod, function = F, arity = A1}, [
    #external_call{module = Mod, function = F, arity = A2} | _
]) ->
    A1 == A2 orelse A2 == undefined;
check_function_ref(#function_def{function = F, arity = A1}, [
    #function_def{function = F, arity = A2} | _
]) ->
    A1 == A2 orelse A2 == undefined;
check_function_ref(#function_def_mod{module = Mod, function = F, arity = A1}, [
    #function_def_mod{module = Mod, function = F, arity = A2} | _
]) ->
    A1 == A2 orelse A2 == undefined;
...
check_var_pattern([], _, _, _, _) ->
    false;
check_var_pattern(
    [#var_pattern{vardefref = VL, function = F, arity = A, clause = C} | Rest], V, F, A, C
) ->
    ?D({VL, F, A, C}),
...
check_var_pattern(
    [#var_pattern{vardefref = VL, function = '', arity = -1, clause = ""} | Rest], V, F, A, C
) ->
    ?D({VL, F, A, C}),
    case lists:member(V, VL) of
        true -> true;
