...
ttb_meta_tracer_loop(MetaFile, PI, Acc, State) ->
    receive
        ...
        {trace_ts, CallingPid, return_from, {erlang, spawn_opt, _Arity}, Ret, _} ->
            NewPid =
                case Ret of
                    {ANewPid, _Mref} when is_pid(ANewPid) -> ANewPid;
                    ANewPid when is_pid(ANewPid) -> ANewPid
                end,
            NewAcc = dict:update(
                CallingPid,
                fun([H | T]) ->
            ...
            ttb_meta_tracer_loop(MetaFile, PI, NewAcc, State);
        {trace_ts, CallingPid, call, {erlang, Spawn, [M, F, Args]}, _} when
            Spawn == spawn; Spawn == spawn_link
        ->
            MFA = {M, F, length(Args)},
            NewAcc = dict:update(
                CallingPid,
            ...
            ttb_meta_tracer_loop(MetaFile, PI, NewAcc, State);
        {trace_ts, CallingPid, return_from, {erlang, Spawn, _Arity}, NewPid, _} when
            Spawn == spawn; Spawn == spawn_link
        ->
            NewAcc =
                dict:update(
        ...
        overload_check ->
            ...
            case catch M:F(check) of
                true ->
                    ...
                    ttb_meta_tracer_loop(
                        MetaFile, PI, Acc, lists:keydelete(overload_check, 1, State)
                    );
                _ ->
                    erlang:send_after(Ms, self(), overload_check),
                    ttb_meta_tracer_loop(MetaFile, PI, Acc, State)
...
pnames() ->
    Processes = processes(),
    Globals = lists:map(
        fun(G) -> {global:whereis_name(G), G} end,
        global:registered_names()
    ),
    lists:flatten(
        lists:foldl(
            fun(Pid, Acc) -> [pinfo(Pid, Globals) | Acc] end,
            [],
            Processes
        )
    ).
...
ttb_stop(MetaPid) ->
    ...
    receive
        {'DOWN', Ref, process, MetaPid, _Info} -> ok
    end,
    stop_seq_trace().
