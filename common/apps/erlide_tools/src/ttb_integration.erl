...
get_file_info(Path) ->
    Result =
        (catch ttbe:format(Path, [{handler, {create_info_handler(Path), {true, 0, '_', '_'}}}])),
    case Result of
                ...
                case Trace of
                    {trace_ts, Pid, call, {Mod, Fun, Args}, Time} ->
                        erlide_jrpc:event(
                            trace_event,
                            {trace_ts, Pid, call,
                                {Mod, Fun, [avoid_interpreting_as_string] ++ Args},
                                calendar:now_to_local_time(Time)}
                        );
                    {trace_ts, Pid, spawn, Pid2, {M, F, Args}, Time} ->
                        erlide_jrpc:event(
                            trace_event,
                            {trace_ts, Pid, spawn, Pid2,
                                {M, F, [avoid_interpreting_as_string] ++ Args},
                                calendar:now_to_local_time(Time)}
                        );
                    {trace_ts, _, _, _, Time} ->
...
create_info_handler(Path) ->
    fun(_Fd, Trace, _TraceInfo, State) ->
        ...
        case Trace of
            end_of_trace ->
                case First of
                    ...
                    false ->
                        erlide_jrpc:event(
                            trace_event,
                            {file_info, calendar:now_to_local_time(Start_date),
                                calendar:now_to_local_time(End_date), Path, Count}
                        )
                end;
            {trace_ts, _, _, _, Time} ->
                case First of
            ...
            _ ->
                {First, Count + 1, Start_date, End_date}
        end
    end.
    ...
    try (str2fun(S)) of
        ...
        {ok, Fun} ->
            try dbg:fun2ms(Fun) of
                ...
                error:function_clause -> {error, not_fun}
            end
    catch
