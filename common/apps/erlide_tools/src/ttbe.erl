...
-export([
    ...
    tpl/2, tpl/3, tpl/4,
    ctpl/0, ctpl/1, ctpl/2, ctpl/3,
    ctpg/0, ctpg/1, ctpg/2, ctpg/3
]).
            ...
            fun
                ({N, {local, File}, TF}, {CS, S}) ->
                    ...
                    case catch dbg:tracer(N, port, dbg:trace_port(ip, 0)) of
                        ...
                        Other ->
                            display_warning(N, {cannot_open_ip_trace_port, Host, Other}),
                            {CS, S}
                    end;
                ({N, C, _} = Client, {CS, S}) ->
...
store(Func, Args) ->
    Last =
        case ets:last(?history_table) of
            '$end_of_table' -> 0;
...
write_config(ConfigFile, Nums, Opt) when
    is_list(Nums), is_integer(hd(Nums));
    Nums =:= ['_']
->
    F = fun(N) ->
        ets:select(
...
proc(Name) when is_atom(Name) ->
    % can be registered on this node or other node
    [Name];
...
stop_opts(Opts) ->
    ...
    case {FormatData, lists:member(return, Opts)} of
        {false, true} ->
            % if we specify return, the data should be fetched
            {fetch, FetchDir};
        {false, false} ->
...
format(File, Out, Handler, DisableSort) when is_list(File), is_integer(hd(File)) ->
    Files =
        case filelib:is_dir(File) of
            % will merge all files in the directory
            true ->
...
format(Files, Out, Handler, DisableSort) when is_list(Files), is_list(hd(Files)) ->
    ...
    Details = lists:foldl(
        fun(File, Acc) -> [prepare(File) | Acc] end,
        [],
        Files
    ),
    Fd = get_fd(Out),
...
format_opt(Opt) when is_list(Opt) ->
    Out =
        case lists:keysearch(out, 1, Opt) of
            {value, {out, O}} -> O;
            _ -> standard_io
        end,
    Handler =
        case lists:keysearch(handler, 1, Opt) of
            {value, {handler, H}} -> H;
            _ -> undefined
        end,
...
check_exists(File) ->
    case file:read_file_info(File) of
        ...
        _ -> exit({error, no_file})
    end.

do_format(Fd, Details, DisableSort, Handler) ->
    Clients = lists:foldl(
        fun({FileOrWrap, Traci}, Acc) ->
...
ip_to_file(Trace, {{file, File}, ShellOutput}) ->
    %File can be a filename or a wrap spec
