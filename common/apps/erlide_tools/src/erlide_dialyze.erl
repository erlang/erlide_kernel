...
update_plt_with_additional_paths(FileName, Paths) ->
    ...
    case update_plt(FileName, Paths, [], []) of
        ...
        {error, no_such_file} ->
            Msg = io_lib:format(
                "Could not find the PLT: ~s\n~s",
                [FileName, default_plt_error_msg()]
            ),
            error(Msg);
        {error, not_valid} ->
            Msg = io_lib:format(
                "The file: ~s is not a valid PLT file\n~s",
                [FileName, default_plt_error_msg()]
            ),
            error(Msg);
        {error, read_error} ->
            Msg = io_lib:format(
                "Could not read the PLT: ~s\n~s",
                [FileName, default_plt_error_msg()]
            ),
            error(Msg);
        {error, {no_file_to_remove, F}} ->
            Msg = io_lib:format(
                "Could not remove the file ~s from the PLT: ~s\n",
                [F, FileName]
            ),
            error(Msg)
    end.
    ...
    case filelib:is_regular(File) of
        ...
        true ->
            case dialyzer_utils:get_abstract_code_from_beam(File) of
                error ->
                    Msg = io_lib:format(
                        "Could not get abstract code for file: ~s (please recompile it with +debug_info)\n",
                        [File]
                    ),
...
expand_dependent_modules(Md5, DiffMd5, ModDeps, Paths) ->
    ...
    {[find_file(Paths, F) || {F, _} <- Md5, FilterFun(F)], RemovedMods, NewModDeps}.

clean_plt(PltFile, RemovedMods) ->
    %% Clean the plt from the removed modules.
    Plt = dialyzer_plt:from_file(PltFile),
    R = sets:fold(
        fun(M, AccPlt) -> dialyzer_plt:delete_module(AccPlt, M) end,
        Plt,
        RemovedMods
    ),
    R.
...
-record(cl_state, {
    ...
    report_mode = normal,
    % :: dial_ret(),
    return_status = ?RET_NOTHING_SUSPICIOUS,
...
failed_anal_msg(Reason, LogCache) ->
    ...
    case LogCache =:= [] of
        true -> Msg;
        false -> Msg ++ "Last messages in the log cache:\n  " ++ format_log_cache(LogCache)
    end.
...
%%  ok;
print_unknown_behaviours(#cl_state{
    output = Output,
    ...
    case
        ordsets:is_element(?WARN_BEHAVIOUR, LegalWarnings) andalso
            DupBehaviours =/= []
    of
        false ->
            ok;
        true ->
            ...
            case Format of
                formatted ->
                    io:put_chars(
                        Output,
                        "Unknown behaviours (behaviour_info(callbacks)"
