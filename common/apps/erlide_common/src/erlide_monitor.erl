...
handle_info({monitor, GcPid, long_gc, Info}, State) ->
    case lists:keyfind(timeout, 1, Info) of
        {timeout, Time} when Time > ?GC_TIME_KILL_LIMIT ->
            erlide_log:log(
                {gc_killing, GcPid, process_info(GcPid, registered_name),
                    process_info(GcPid, heap_size)}
            ),
            %%      erlang:kill(GcPid),
            ok;
        _ ->
    ...
    {noreply, State};
handle_info(
    {monitor, GcPid, large_heap, Info}, #state{warnLimit = WarnLimit, killLimit = KillLimit} = State
) ->
    case lists:keyfind(heap_size, 1, Info) of
        {heap_size, Size} when Size > KillLimit ->
            erlide_log:log(
                warn,
                {heap_killing, GcPid, process_info(GcPid, registered_name),
                    process_info(GcPid, heap_size)}
            ),
            %%       erlang:kill(GcPid),
            send_info(),
        ...
        {heap_size, Size} when Size > WarnLimit ->
            erlide_log:log(
                warn,
                {heap_warning, GcPid, process_info(GcPid, registered_name),
                    process_info(GcPid, heap_size)}
            ),
            send_info(),
            ok;
